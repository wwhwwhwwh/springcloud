 ...
 * @since 2018/05/02
 */
public class FiveTimesRoundRule extends AbstractLoadBalancerRule {
    ...
    public FiveTimesRoundRule() {
        nextServerCyclicCounter = new AtomicInteger(0);
        toTalCounter = new AtomicInteger(0);
    }

    public FiveTimesRoundRule(ILoadBalancer lb) {
        this();
        setLoadBalancer(lb);
    }
    ...
    private int incrementAndGetModulo(int modulo) {
        for (; ; ) {
            if (toTalCounter.get() < 5) {
                ...
                toTalCounter.compareAndSet(currenttoTal, next);
                return nextServerCyclicCounter.get();
            } else {
                toTalCounter.compareAndSet(5, 0);
